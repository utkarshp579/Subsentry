...
const UpcomingRenewals: React.FC<UpcomingRenewalsProps> = ({ subscriptions }) => {
  // Filter and sort renewals in next 30 days
  const now = new Date();
  ...
  if (upcoming.length === 0) {
    return (
      <Card className="p-8 rounded-2xl bg-linear-to-br from-[#18181c] to-[#101010] border border-[#232323] text-center shadow-lg">
        <span className="text-gray-400 text-base">No upcoming renewals in the next 30 days.</span>
      </Card>
    );
  }
            ...
            >
              <div className="flex items-center gap-3 min-w-0 flex-1">
                {icon && (
                  ...
                  >
                    <span className={`w-6 h-6 ${colors?.icon ?? "text-white"}`}>{icon}</span>
                  </span>
                )}
                <div className="flex flex-col min-w-0">
                  <span className="font-medium text-white truncate text-base">{sub.name}</span>
                  <span className="text-xs text-gray-400 mt-1">
                    {format(new Date(sub.renewalDate), "MMM d, yyyy")} &bull; {sub.billingCycle}
                  </span>
                </div>
              </div>
              <div className="flex flex-col items-end">
                <span className="font-semibold text-white text-lg">₹{sub.amount}</span>
                <span
